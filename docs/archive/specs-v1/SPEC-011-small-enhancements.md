...
|----|------|-------|--------|
| SPEC-011-A | IDEA-039 | Stations of the Cross: replace V/R label divs with inline styled text | done |
| SPEC-011-B | IDEA-056 | Event detail card: phone number as tappable call link | done (already implemented) |
...
- **Date:** 2026-03-12
- **Status:** done
- **Files changed:** `src/stations.js` — replaced 8-line block div V/R markup with 2-line inline `<p>` + `<span>` approach; `css/app.css` lines 1876-1880 — replaced flexbox card layout with inline paragraph styles, removed `.stations-v-label/.stations-r-label/.stations-v-text/.stations-r-text` selectors, added `.stations-vr-label` and `.stations-vr-label--response`; dark mode override removed (uses CSS variables that auto-switch)
- **Approach:** V/R markers became inline `<span>` elements inside `<p>` tags. V label uses `--color-accent` (season-aware — purple in Lent), R label uses `--color-text-secondary`. Removed the background card styling from versicle/response blocks to further reduce vertical space.
- **Deviations from spec:** Used `var(--space-1)` instead of `var(--space-xs)` for margins since `--space-xs` is not defined in the token set. Used `var(--weight-bold)` token instead of raw `700`.
...
- **Files changed:** None — feature was already in place
- **Approach:** Parish detail Call button (render.js line 460) already uses `tel:` link. Event contact phones (events.js lines 407-414) already use `tel:` links with 10+ digit validation. No changes needed.
- **Deviations from spec:** None — the spec described what was already built.
- **Known issues:** None observed.
