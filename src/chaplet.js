...
reader.registerModule('chaplet', {
  ...
  render: function(params, bodyEl, footerEl) {
    ...
    _load().then(function() {
      ...
      _visHandler = prayerCore.wakeLock.onVisibility('chaplet');
      document.addEventListener('visibilitychange', _visHandler);
      _swipeTeardown = prayerCore.initSwipe(
        function() { if (_screen !== 'intro') chapletNext(); },
        function() { if (_screen !== 'intro') chapletPrev(); }
      );
    });
  },
  onClose: function() {
...
// Wake lock, fmtPrayer, crossfade, swipe extracted to prayer-core.js (ARC-04)
...
function chapletNext() {
  ...
  if (_screen === 'intro') {
    ...
    prayerCore.logCompletion('chaplet');
    _haptic.confirm();
    closeChaplet();
    return;
  }
...
// Crossfade + swipe now via prayer-core.js
...
function _renderIntro(title, body, footer) {
  ...
  body.innerHTML = '<div class="prayer-splash">'
    ...
    + '<h2 class="prayer-splash-title">The Divine Mercy Chaplet</h2>'
    + '<p class="prayer-splash-subtitle">' + utils.esc(_t(_chaplet, 'origin')) + '</p>'
    + '<p class="prayer-splash-desc">\u201C' + utils.esc(_chaplet.quote.text) + '\u201D<br><cite style="font-size:var(--text-xs);color:var(--color-text-tertiary);font-style:normal">\u2014 ' + utils.esc(_chaplet.quote.ref) + '</cite></p>'
    + '<button class="prayer-splash-begin" onclick="chapletNext()">Begin</button>'
    + '</div>';
}
